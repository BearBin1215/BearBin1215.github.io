...
const BASE_TIME = 400;
/** 速度/加工室插件数量（行、列数，0-24 共 25 级） */
const PLUGIN_COUNTS = 24;
/** 待处理物品总数 */
const ITEM_COUNT = 64;
/** 插件等级上限 */
const MAX_LEVEL = 9;
...
function calculateTime(
  ...
  /** 单配方耗时（tick，向上取整） */
  const oneItemTick = Math.ceil(BASE_TIME / speedMultiplier);
  /** 总并行数 */
...
export default function OritechCalculator() {
  const [speedLevel, setSpeedLevel] = useState(9);
  const [processLevel, setProcessLevel] = useState(9);
  const [selectedCount, setSelectedCount] = useState(4);
  useDocumentTitle("Oritech 效率计算");

  /**
   * 加工室行的最大数量：首个并行数达到堆叠上限(64)的配置之后，再增加加工室插件
   * 并行数溢出无意义（每轮至多处理 64 个），用于隐藏表格下方的多余行。
   */
  const maxProcess = Math.min(
    PLUGIN_COUNTS - 1,
    processLevel === 0 ? PLUGIN_COUNTS - 1 : Math.ceil((ITEM_COUNT - 1) / processLevel),
  );
  ...
  const minTimeInfo = useMemo<MinTimeInfo>(() => {
    ...
    for (
      let i = Math.max(0, selectedCount - maxProcess);
      i <= Math.min(selectedCount, PLUGIN_COUNTS - 1);
      i++
    ) {
  ...
  }, [speedLevel, processLevel, selectedCount, maxProcess]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Table className="border-collapse bg-card/52">
        <TableHeader>
          <TableRow>
          ...
          <TableRow>
            {pluginList.map((speed) => (
              ...
              >
                ...
                <br />
                耗时{Math.ceil(BASE_TIME / (1 + speed * (speedLevel * 0.5)))}t
              </TableHead>
            ))}
          </TableRow>
        </TableHeader>
        <TableBody>
          {pluginList
            .filter((process) => process <= maxProcess)
            .map((process) => (
